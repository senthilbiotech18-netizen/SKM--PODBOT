...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f4] flex flex-col items-center justify-center p-6 font-sans text-[#1c1917]">
      ...
      <div className="mt-8 text-stone-400 text-[10px] text-center max-w-xs leading-relaxed space-y-1">
        <p>Explain what you've learned. Your tutor will guide you through any topic you choose.</p>
        {logs.length > 0 && (
          <div className="pt-2 border-t border-stone-200 mt-2 text-left bg-stone-100 p-2 rounded relative">
            <div className="flex justify-between items-center mb-1">
              <span className="text-[8px] text-stone-400 uppercase font-bold">Debug Logs</span>
              <div className="space-x-2">
                <button 
                  onClick={copyLogs}
                  className="text-[8px] text-indigo-500 hover:text-indigo-700 uppercase font-bold"
                >
                  Copy
                </button>
                <button 
                  onClick={() => setLogs([])}
                  className="text-[8px] text-stone-400 hover:text-stone-600 uppercase font-bold"
                >
                  Clear
                </button>
              </div>
            </div>
            {logs.map((log, i) => <div key={i} className="truncate text-[9px]">• {log}</div>)}
          </div>
        )}
      </div>
    </div>
